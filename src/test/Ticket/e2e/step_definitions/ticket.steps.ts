...
const feature = loadFeature("src/test/Ticket/e2e/features/ticketform.feature");
...
defineFeature(feature, (test) => {
  ...
  test("Submitting either a depsoit or withdraw form on the profile page", ({
    given,
    when,
    then,
  }) => {
    given("the user is on the profile page", async () => {
      await driver.get("http://localhost:3000/profile/testUser/deposit-ticket");
    ...
    when(
      ...
      async () => {
        const inputName = await driver.findElement(
          By.css("[data-testid='characterNameInput']")
        );
